Public Class NeuralNet
    ...
    Sub TrainNet(Optional TrainingBatch As Batch = Nothing)
        ...
        If TrainingBatch Is Nothing Then
            ...
            If Reward <> 0 Or IsBoardFull(CheckAction) Then
                NextMaxQ = 0
            End If

            Exploration = 0
            CheckAction = TrainingState.Clone
            FeedForward(CheckAction.Clone, True, True)
            ...
            Next
        Else
            ' fix this and what happens if they win game early
            TrainingState = TrainingBatch.State.Clone
            Dim Training As Boolean = True
            Exploration = 0

            If TrainingBatch.Reward <> 0 Or IsBoardFull(TrainingBatch.NewState) Then
                NextMaxQ = 0
            Else
    ...
    Sub FindReward()
        ...
        If Module1.WInX And Team = "1" Then
            Reward = 0.9
            '  NumOfWins += 1
            'Module1.TrainingNet.NumOfLoss += 1

            If Iteration > 399000 Then
                NumOfWins += 1
            End If
        ElseIf Module1.Win0 And Team = "1" Then
            Reward = -0.9
            '  NumOfLoss += 1
            ' Module1.TrainingNet.NumOfWins += 1
            If Iteration > 399000 Then
                NumOfLoss += 1
            End If
        ElseIf Module1.WInX And Team = "-1" Then
            Reward = -0.9
            ' Module1.Net.NumOfWins += 1
    ...
    Sub PlayGame()
        ...
        If Iteration < 1000 Then
            Module1.TrainingNet.Threshold = -1
            Module1.Net.Threshold = -1
        Else
            Module1.TrainingNet.Threshold = 0.8
            Module1.Net.Threshold = 0.8
        End If
        'If Team = "1" Then
        ...
        If Iteration >= 399000 Then
            Module1.Net.Threshold = 2
            Module1.TrainingNet.Threshold = 2
        ...
        If Team = "-1" Then
            Module1.Net.Exploration = Rnd.Next(0, 100) / 100
        ...
        While InGame
            ...
            If IsBoardFull(CheckAction) Or WInX Or Win0 Then
                InGame = False
            End If
            ...
            If Module1.WInX = True Or Module1.Win0 = True Or IsBoardFull(Module1.State) = True Then
                For y = 0 To 2
                    For x = 0 To 2
                        Module1.State(x, y) = "."
                    Next
