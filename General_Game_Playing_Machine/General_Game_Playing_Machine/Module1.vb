Module Module1
    ...
    Sub XORProblem()
        ...
        Do
            ...
            Dim OutputDerivative As Double = derivative(OutputNeuron)

            Dim OutputError As Double = (Target - OutputNeuron) * OutputDerivative

            Dim HiddenError(NoOfHiddens - 1) As Double

            For x = 0 To NoOfHiddens - 1
                HiddenError(x) = HOWeights(x) * OutputError * derivative(OutputNeuron)
            Next

            Dim InputError(NoOfInputs - 1) As Double


            For y = 0 To NoOfInputs - 1
                For x = 0 To NoOfHiddens - 1
                    IHWeights(x + y * NoOfHiddens) += LearningRate * HiddenError(y) * InputNeurons(y)
                Next
            Next

            For y = 0 To NoOfHiddens - 1
                HOWeights(y) += LearningRate * OutputError * HiddenNeurons(y)
            Next
            Count += 1
